...
vars<-colsplit(datarenamed$sample,"_",c("species","well","sample.id", "loc","seq_direction")) #separate sample name by metadata factors that are in the name of your sample- CHANGE THIS FOR YOUR DATA
...
levels(samplesonly$sample.id<-factor(samplesonly$sample.id))#double check for typos in categories
