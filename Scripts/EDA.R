...
datarenamed<-read.table("linecount_bysample.txt", header=T, sep="") #read in the line counts file you generated in Step 3 of workflow, using the raw data
datarenamed$log_No_seqs<-log(datarenamed$No_seq)

samplesonly<-subset(datarenamed, species !="emptywell") #do this if you have any wells that were blanks, if not then you don't have to subset
levels(samplesonly$species<-factor(samplesonly$species))#double check for typos in categories

emptywells<-subset(datarenamed, species =="emptywell") #Use this is you have blank wells
...
#Summaries by plate:
mean<-tapply(samplesonly$No_seq, samplesonly$plate,mean) #if you did not subset to remove empty wells in line 50, then do this for the object "datarenamed" instead of "samplesonly"
...
sample.reads.binRA<-subset(sample.reads.bin,sequence_direction=="RA.fastq") #subset out only forward reads
...
pdf("No.Reads_binned.pdf", width=10, height=6)
...
pdf("No. Reads Binned LOW.pdf", width=10, height=6)
...
#NOTE that these columns may not be appropriate for your dataset. The point here is to look at patterns pertaining to metadata, so these might include collection date, extraction method, etc.

plot(failed$Year_Collected,failed$No_seq,las=2)
plot(failed$Loc_combined,failed$No_seq,las=2)
plot(failed$Sex,failed$No_seq,las=2)
plot(failed$Capture_type,failed$No_seq,las=2)
plot(failed$Collection_Method,failed$No_seq,las=2)
plot(failed$Stage_Class,failed$No_seq,las=2)

#####II. Mapping Stats########
#Comparing filtered (rmdup) and unfiltered####
#Read in data and combine####
library(reshape2)
list.files(pattern="All_head") #list files with this pattern in the title
mapped.raw<-read.delim("All_head_flagstat_reformat.txt",header=T) #read in file generated after mapping
mapped.fltr<-read.delim("All_head_sortfltr_flagstat_reformat.txt",header=T) #read in file generated after removing PCR dups
...
mapped.comb<-mapped.comb[,c(1,3,15,4,16,5,17,6,18,7,19,8,20,9,21,10,22,11,23,12,24,13,25)] #reorganize dataframe, this can be changed if needed
vars<-colsplit(mapped.comb$ID, "_", c("species","LABID","DID","plate","well")) #the options after c() will be specifiy to your naming scheme for your samples, these are all included in the sample names here
...
#This section may not be appropriate for workflows aiming to do SNP discovery, as this requires relative known positions present in each sample
